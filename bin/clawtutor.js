...
/**
 * Shell wrapper for ClawTutor CLI
 *
 ...
 * - clawtutor-linux-x64
 * - clawtutor-darwin-arm64
 * - clawtutor-darwin-x64
 * - clawtutor-windows-x64
 */
...
function runBinary(binaryPath, packageRoot) {
  const child = spawn(binaryPath, process.argv.slice(2), {
    ...
    env: {
      ...process.env,
      CLAWTUTOR_PACKAGE_ROOT: packageRoot,
      CLAWTUTOR_PACKAGE_JSON: join(packageRoot, 'package.json'),
    },
  });
...
const platformMap = {
  'linux-x64': { pkg: 'clawtutor-linux-x64', bin: 'clawtutor' },
  'linux-arm64': { pkg: 'clawtutor-linux-arm64', bin: 'clawtutor' },
  'darwin-arm64': { pkg: 'clawtutor-darwin-arm64', bin: 'clawtutor' },
  'darwin-x64': { pkg: 'clawtutor-darwin-x64', bin: 'clawtutor' },
  'win32-x64': { pkg: 'clawtutor-windows-x64', bin: 'clawtutor.exe' },
};

const envBinary = process.env.CLAWTUTOR_BIN_PATH;
if (envBinary) {
  const resolvedEnvBinary = isAbsolute(envBinary) ? envBinary : resolve(process.cwd(), envBinary);
  if (!existsSync(resolvedEnvBinary)) {
    console.error(`Error: CLAWTUTOR_BIN_PATH target not found (${resolvedEnvBinary})`);
    process.exit(1);
  }
  ...
  const searchPaths = [
    ...
    // 3. Global install location (try to resolve from parent of clawtutor)
  ...
  if (!binaryPath) {
    ...
    console.error(`\nTry reinstalling: npm install -g clawtutor`);
    console.error(`\nPlease report this issue at: https://github.com/moazbuilds/ClawTutor-CLI/issues`);
    process.exit(1);
  }
